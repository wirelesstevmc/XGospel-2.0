...
# Clean 32-bit build script for XGospel without modern* code
CC="gcc"
...
echo "Building clean 32-bit XGospel..."
...
for src in except.c mymalloc.c actions.c showwidgets.c myxlib.c myxinternals.c mytext.c TMyprint.c mycontext.c myconverters.c myclass.c YShell.c Canvas.c Tree.c SmeToggle.c SmeLabel.c Popup.c TearofMenu.c; do
    if [ -f "$src" ]; then
        echo "Compiling $src..."
...
if [ -f xgospel ]; then
    echo "Build successful! Binary created: xgospel"
    ls -la xgospel
else
    echo "Build failed!"
    exit 1
